...
def list_admin_accounting_entries(db, limit=500):
    """
    Admin-safe accounting entry list.

    Rules:
    - Return [] if accounting_entries table does not exist.
    - Return [] if SQL fails.
    - Never crash /admin/accounting.
    """
    if not _accounting_table_exists(db):
        return []
    ...
    try:
        ...
        print(f"[ADMIN_ACCOUNTING][ENTRIES][ERROR] {exc}")
        return []


def admin_accounting_summary(db, *, date_text=None, limit=300):
    """
    Admin accounting dashboard summary for /admin/accounting.

    Compatibility goals:
    - Works when accounting_entries is empty.
    - Works when accounting_entries table does not exist.
    - Tolerates NULL amount_twd.
    - Tolerates direction values:
      INFO / CREDIT / DEBIT / IN / OUT.
    - Returns all keys required by templates/mobile/admin/accounting.html.
    """
    today = _date_text(date_text or now_iso())
    month_start, next_month_start = _current_month_range()
    ...
    entries = list_admin_accounting_entries(db, limit=limit)
    summary["recent_entries"] = entries
    summary["entries_count"] = len(entries)

    for row in entries:
        amount = _money(_row_get(row, "amount_twd", 0))
        role = _text(_row_get(row, "role", "UNKNOWN"), "UNKNOWN").upper()
        direction = _text(_row_get(row, "direction", "UNKNOWN"), "UNKNOWN").upper()
        entry_type = _text(_row_get(row, "entry_type", ""), "").upper()
        created_at = _text(_row_get(row, "created_at", ""))

        summary["total_amount_twd"] += amount

        if role not in summary["by_role"]:
            summary["by_role"][role] = 0
        summary["by_role"][role] += amount

        if direction not in summary["by_direction"]:
            summary["by_direction"][direction] = 0
        summary["by_direction"][direction] += amount

        if created_at.startswith(today):
            summary["today_entries_count"] += 1
            summary["today_amount_twd"] += amount

        if _date_in_range(created_at, month_start, next_month_start):
            summary["month_entries_count"] += 1
            summary["month_amount_twd"] += amount

        # Current accounting_service.py entry types.
        if entry_type == "STORE_PLATFORM_FEE":
            summary["store_platform_fee_twd"] += amount

        elif entry_type == "DRIVER_PLATFORM_FEE":
            summary["driver_platform_fee_twd"] += amount
            summary["cod_admin_receivable_from_driver_twd"] += amount

        elif entry_type == "PLATFORM_FEE":
            summary["admin_revenue_twd"] += amount

        # Future/backward-compatible entry types used by richer accounting pages.
        elif entry_type == "PLATFORM_CUSTOMER_COLLECTED":
            summary["platform_customer_collected_twd"] += amount

        elif entry_type in {"PLATFORM_PAY_STORE_NET", "ADMIN_PAYOUT_STORE"}:
            summary["platform_pay_store_twd"] += amount

        elif entry_type in {"PLATFORM_PAY_DRIVER_NET", "ADMIN_PAYOUT_DRIVER"}:
            summary["platform_pay_driver_twd"] += amount

    # If PLATFORM_FEE rows do not exist, calculate admin revenue from fee components.
    if summary["admin_revenue_twd"] <= 0:
        summary["admin_revenue_twd"] = (
            summary["store_platform_fee_twd"]
            + summary["driver_platform_fee_twd"]
        )

    summary["platform_keep_revenue_twd"] = summary["admin_revenue_twd"]
    ...
    try:
        row = db.execute(
            ...
            SELECT COUNT(*) AS c
            FROM orders
            WHERE COALESCE(admin_hold, 0) = 1
               OR payment_status IN ('PENDING', 'PENDING_REUPLOAD')
               OR status = 'DISPUTED'
               OR payment_method = 'PREPAID_TO_STORE'
            """
        ).fetchone()

        summary["manual_review_orders"] = int(row["c"] or 0) if row else 0

    except Exception as exc:
        print(f"[ADMIN_ACCOUNTING][MANUAL_REVIEW_COUNT][ERROR] {exc}")
        summary["manual_review_orders"] = 0
